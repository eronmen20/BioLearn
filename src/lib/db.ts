...
export async function createUser(email: string, password: string, name: string) {
  ...
  const { error } = await supabase.from("users").insert({ id, email, password: hash, name, role: "user" });
...
export async function getProgress(userId: string) {
  const { data: rows } = await supabase.from("progress").select("*").eq("user_id", userId);
